(() => {
  const authConfig = {
    apiKey: 'AIzaSyCLWf5X5fKnjpHnSxBB5Kgdd-VmdMJuOVA',
    authDomain: 'pendientes-6c0c8.firebaseapp.com',
    projectId: 'pendientes-6c0c8'
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const signOut = async () => { await auth.signOut(); location.reload(); }; document.getElementById('guestAccess').addEventListener('click', () => { gate.remove(); const status = document.getElementById('masterStatusText'); if (status) status.textContent = 'Modo invitado: Base Maestra no disponible.'; });
    ...
    auth.onAuthStateChanged(async (user) => {
      if (!user) return;
      const snapshot = await roleRef(user.uid).once('value');
